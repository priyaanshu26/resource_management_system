...
export async function DELETE(
    ...
    try {
        ...
        // Check if building has associated resources
        const building = await prisma.building.findUnique({
            where: { id },
            include: {
                _count: {
        ...
        if (building._count.resources > 0) {
            return NextResponse.json(
                { error: `Cannot delete building with ${building._count.resources} associated resources` },
                { status: 400 }
            );
        }
